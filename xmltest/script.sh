...
SECURITY_CONFIG_DIR=/var/security


if [ -f $SECURITY_CONFIG_DIR/"security.json" ]; then
  bin/solr zk cp file:${SECURITY_CONFIG_DIR}/security.json zk:/security.json -z ${ZK_HOST}
fi

cp -f $TEMP_CONFIG_DIR/"solr.xml" $HOME_DIR/"solr.xml"
  ...
  #  # Add your commands or code here
  #done
...
#    diff_xml_lines=("${diff_xml_lines[@]:0:$i1}" "${diff_xml_lines[@]:$((i2+1))}")
    ...
    if [[ "$i1" -ne -1 ]]; then
      for((i=i1+1; i<target_length; i++)); do
        ...
        echo "checking for difffname $i ${diffName}"
        echo ""
        if [[ "${diffName}" == "shardHandlerFactory" ]]; then
          i2=$i
          break
      ...
      echo "printing shard handler block $i1 $i2"
      echo "${target_xml_lines[@]:$i1:$((i2+1))}"
      echo "printed shard handler block"
...
#  end of backup ***************************************************
  ...
  echo "${target_xml_lines[@]}" > "temp.xml"

  xmllint --format "temp.xml" > "out.xml"
